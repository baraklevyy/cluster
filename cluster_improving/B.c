...
void b_mult(const struct _spmat *A, const int *k, int M, double *v, double *res, const double *f, double l1_norm){
    ...
    for(i = 0; i < n; i++){
        temp_res = 0.0;
        temp_res -= (tmp * (*(k + i)) / ((double)M));
        temp_res -= (*(f + i)) * (*(v + i));
        temp_res += l1_norm * (*(v + i));
        *(res + i) = temp_res;
...
void power_iteration_modified(const struct _spmat *A, const int *k, int M, double L1norm, double *res, double *f, double *random_vector) {
    ...
    *(random_vector) = 0;
    ...
    double *ggg = (double *)malloc(n*sizeof(double ));
	generate_random_normalize_vector(ggg, n);
    ...
    while (1) {/*two optimization has to deal here 
        ...
        b_mult(A, k, M, ggg, res, f,L1norm);
        vec_normalize(res, n);
        if (diff_below_epsilon(ggg, res, n))
            break;
        for (i = 0; i < n; i++)
            *(ggg + i) = *(res + i);
    }
...
void power_iteration_eigval_modified(const struct _spmat *A, const int *k, int M, double *f,
    ...
    Bgag_mul_eig_vec = (double *) calloc(n,  sizeof(double));
    b_mult(A, k, M, Bgag_mul_eig_vec, eig_vec, f, L1norm);
...
void rows_arrangment(struct _spmat *A, double *s, int number_of_1, int *rows_helper, node ***outer_array_helper) {
    ...
    for (i = 0; i < n; i++) {
        ...
        *(*(outer_array_helper) + (cur_outer_arr_index)) = *(outer_array + i);
        /*Initializing starting nodes*/
        current_node = *(outer_array + i);
        previous_node = *(outer_array + i);
        while (current_node != NULL) {
            ...
            if (((*(s + i) == 1.0) && (*(rows_helper + cur_col) >= k2_start_index)) ||
                ...
                *(A->onces_num + cur_outer_arr_index) -= 1;
                if(current_node == *(outer_array + i)) { /* The first element in the list should be deleted*/
